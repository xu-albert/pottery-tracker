...
- [x] **Firebase sync (Phase 2)** — Google/Apple auth backend, Firestore, Cloud Storage
...
- [x] **Widget tests** — run `flutter test` to execute the widget tests covering album screen, filter chips, album grid, archive thumbnails, empty state, and settings
- [x] **CI/CD** — GitHub Actions for `dart analyze` + `flutter test` on PRs, plus a Cloud Function typecheck + test job
