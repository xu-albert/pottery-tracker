...
class SyncTrigger {
  ...
  Future<void> afterPhotoWrite(String photoId,
      {bool includeFile = false}) async {
    await _queue.enqueue(SyncQueueEntry(
  ...
  Future<void> afterPieceDeletion(
      String pieceId, List<String> photoIds) async {
  ...
  Future<void> afterMaterialDeletion(
      String collection, String materialId) async {
    await _queue.enqueue(SyncQueueEntry(
