...
Piece and photo writes from screens go through `PieceWriter` (`lib/services/piece_writer.dart`), whose tests pin the enqueue for each operation; the album swipe-to-archive in `album_grid.dart` is the one remaining direct DAO write.
