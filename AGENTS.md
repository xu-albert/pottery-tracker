...
A run decides from its own outcome, never from the latched `SyncState.status`: the status is what the *previous* run left behind, so a drain that read it kept a device which had pushed everything showing "Sync error" with a stale message. A successful drain therefore returns the device to idle and advances `lastSyncedAt` even when the latched error came from a failed pull — `syncNow`'s catch is the only writer of the error the tile shows, so holding the clear back for a pull would restore exactly that stale-error bug. The claim speaks for the push half only: a drain never pulls (`_syncOwed` holds it back when a full sync is owed), and a `pushPhotoFile` entry is best-effort, so "backed up" can stand over a photo whose bytes `retryMissingUploads` has yet to deliver. Neither path raises an error for a push it could not send: the entries stay queued and `pendingCount` is what reports them. `SyncState.copyWith` drops `errorMessage` whenever it is not passed — that is how a healthy state stops carrying a dead failure's caption, and several callers depend on it. The same rule, for the same reason, governs the read-only lock below.
