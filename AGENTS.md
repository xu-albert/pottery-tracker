...
A run decides from its own outcome, never from the latched `SyncState.status` — the status is what the *previous* run left behind, so reading it strands stale errors. The one deliberate read is a failed drain's: it replaces the reason of an error already showing, and never decides whether to enter error. `_pushQueue` in `lib/providers/sync_provider.dart` owns the rationale and the limits of what a drain's "backed up" claim covers. `SyncState.copyWith` keeps the current reason through a count update and clears it whenever a status is set without a new one. Failed photo uploads remain pending through the local photo rows (`SyncService.pendingPhotoUploadIds`), even after the best-effort queue attempt is retired. The read-only lock below is governed by persisted ownership, never the failure caption.
