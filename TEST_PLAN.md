...
| `test/database/migration_test.dart` | The real `MigrationStrategy` walked from every historical schema version (discovered from the checked-in DDL in `test/database/fixtures/`, with a case failing if that set is not exactly 1 to `schemaVersion - 1`) to the current one: each upgrade completes, lands on exactly a fresh install's tables and columns, gives `tag_options` one `color` column, and leaves the migrated database writable; a version 1 piece survives with its clay and glaze text backfilled into the libraries and its clays numbered alphabetically | 19 |
