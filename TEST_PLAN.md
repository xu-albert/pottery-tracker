...
| `test/providers/sync_queue_durability_test.dart` | Real Drift + `PieceWriter` + persisted queue: edits during incremental/full-sync pull or upload retry survive and receive an owed drain; an edit during a drain receives a follow-up; exhausted pushes stay queued without a hot retry loop | 6 |
...
|---|---|---|
| `fix: preserve edits queued during sync and repay deferred drains` | Full-sync completion cleared unacknowledged edits and exhausted pushes; a debounce expiring while sync was busy was dropped. Covers the blanket-clear part of pt-sync-durability E1 and discharges E2's owed-reschedule half; same-entry revision acknowledgement remains the separate queue-ack prerequisite | `sync_queue_durability_test.dart` (six controlled-timer regressions; incremental loss and missing follow-up reproduced before the fix), `sync_provider_test.dart` (full sync acknowledges drained entries) |
