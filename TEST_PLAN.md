...
|---|---|---|
| `fix(sync): report a drain's own outcome, not the latched status` | The debounced drain decided idle-vs-error from `SyncState.status`, which is what the *previous* run latched: once anything set the error, a later drain that pushed everything left Settings showing "Sync error" with the stale message and never advanced `lastSyncedAt`, until the user tapped Sync Now or signed in again — while a drain whose retries were all exhausted claimed the device backed up anyway. A drain now answers from whether it pushed everything, and still may not claim a backup while a full sync is owed or its pull failed, since a drain never pulls | `sync_provider_test.dart`: `"a recovered drain returns the device to idle and backed up"`, `"a drain that is still failing claims no backup"`, `"a best-effort photo file failure is not a drain failure"`, `"a drain does not clear an error a failed pull left"` |
