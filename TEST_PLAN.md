...
|---|---|---|
| `fix(sync): preserve current failure reasons and count pending photos` | Count updates dropped the failure reason, failed drains retained a previous failure's caption, and best-effort photo uploads vanished from the pending count after queue retirement. Sync reasons now survive until an attempt resolves or replaces them, and a failed drain replaces the reason of an error already showing without raising one; pending work includes unuploaded local photo rows, deduplicated against file queue entries. A photo stays pending until its URL is published remotely | `sync_provider_test.dart`: count/retry reason lifecycle, replacement by a later drain, successful clear; `sync_photo_pending_test.dart`: full and incremental failure, restart, successful retry, URL-publication failure, debounced upload, no double count and deletion; `settings_sync_tile_test.dart`: an unuploaded photo suppresses "All data backed up" until delivered. The reason and both photo-path regressions failed before the fix |
