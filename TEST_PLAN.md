...
```
  ...
  ╱─────────────╲
 ╱   Unit / DAO   ╲ (§2.1) — 30 files: providers, services, DAOs, pure helpers
╱───────────────────╲
```
...
| `test/providers/sync_provider_test.dart` | `SyncState.copyWith` and notifier plumbing, including the debounced drain deciding idle-vs-error from its own outcome rather than the latched status, and the failure reason a count update keeps while a new attempt or result clears or replaces it — a failed drain only while an error is already showing | 47 |
| `test/providers/sync_offline_pull_test.dart` | Real notifier, service, queue and Drift with a cache/server Firestore boundary: any unavailable collection prevents full/incremental sync success and watermark advancement; reconnect retrieves the remote edit | 14 |
| `test/providers/sync_photo_pending_test.dart` | Real notifier, service, queue and Drift with fake Firestore/Storage, for full and incremental sync: a photo whose upload failed stays pending after its queue attempt is retired and across a restart, counts once while also queued, stays pending when its URL cannot be published remotely, and clears once uploaded or deleted | 2 |
| `test/providers/sync_queue_durability_test.dart` | Real Drift + `PieceWriter` + persisted queue: a first sign-in retires its 36-entry bulk-upload snapshot without individually re-uploading photos, but sends queued tombstones — which no bulk upload of existing rows delivers — before it pulls, and keeps one whose push failed; edits during incremental/full-sync pull or upload retry survive and receive an owed drain; an edit merged into the entry being pushed is not acknowledged by that push; an edit during a drain receives a follow-up; exhausted pushes stay queued without a hot retry loop; offline edits immediately count as pending before the debounce and while an upload waits | 12 |
...
| `test/features/settings/settings_sync_tile_test.dart` | Sync tile against the real notifier, service and queue: an offline launch that cannot pull still reports the work waiting on the device, so does a sync left in progress by a push the device cannot deliver, a later offline edit moves that count in both states before any drain runs, a sync whose push half emptied the queue stops reporting that work while its pull half is still running, and the reason a sync failed is drawn beside the count at the moment it fails — as plain words when that reason is the server being out of reach, never as the Firestore SDK's own text, and still there once a later offline edit has moved the count; and a photo left unuploaded keeps the tile off "All data backed up" until a sync uploads it (what later attempts do with that reason is pinned in `sync_provider_test.dart`) |
...
|---|---|---|
| `fix(sync): preserve current failure reasons and count pending photos` | A drain that failed while an error was already showing left the earlier failure's caption on the tile and never said why it had failed itself, and best-effort photo uploads vanished from the pending count once their queue entry was retired. `SyncState.copyWith` now keeps the current reason through a count refresh and clears or replaces it whenever a status is set, so a new attempt starts without one; a failed drain replaces the reason only while an error is already showing and raises no error otherwise. Pending work includes unuploaded local photo rows, deduplicated against file queue entries, and a photo stays pending until its URL is published remotely | `sync_provider_test.dart`: `"a count update keeps the reason; a status or new reason replaces it"`, `"a new drain failure replaces the previous pull reason"`, `"a drain that is still failing claims no backup"`, `"a retry replaces or clears the reason"`; `sync_photo_pending_test.dart`: `"full/incremental sync keeps failed photos pending across restart and clears them after upload"` (failure, restart, successful retry, URL-publication failure, debounced upload, no double count and deletion); `settings_sync_tile_test.dart`: `"an unuploaded photo prevents the all-backed-up label"`. The drain-replacement case and both photo paths failed before the fix; the retry and still-failing cases pass at base and pin behaviour the fix must keep |
| `fix(sync): keep backup claims honest while offline` | An edit kept the old zero pending count until the drain finished, including while an offline upload waited indefinitely; cache-backed pulls reported success and advanced the per-device watermark past unseen remote edits; and the count the fix keeps current was drawn only in the idle branch of the Settings tile, so neither the offline error state — now the routine one — nor a sync still waiting on a push the device cannot deliver showed any sign of the unsent edit, and a sync reported its pre-push count for as long as its pull half ran. Serving pulls from the server alone also made the error tile the routine offline outcome, captioned with the SDK's own advice to enable offline mode | `sync_queue_durability_test.dart`: offline edit before debounce and during upload; `sync_offline_pull_test.dart`: real notifier, service, queue and Drift, with SDK-boundary cache/server behavior for every collection in full and incremental pulls, unchanged watermark/last-sync time on failure, and recovery after reconnect; `settings_sync_tile_test.dart`: the pending count on the tile in the error and syncing states, including a sync that has already delivered it, and the offline caption an unreachable server gets and keeps across a later edit. 22 of the 23 new cases failed at this commit's base; the exception, `"stops reporting work the running sync has already delivered"`, passes there because the base tile drew no subtitle at all while syncing — it guards the mid-sync count refresh, not the base behaviour |
| `fix(sync): report a drain's own outcome, not the latched status` | The debounced drain decided idle-vs-error from `SyncState.status`, which is what the *previous* run latched: once `syncNow` set the error, a later drain that pushed everything left Settings showing "Sync error" with the stale message and never advanced `lastSyncedAt`, until the user tapped Sync Now or signed in again — while a drain whose retries were all exhausted claimed the device backed up anyway. A drain now answers from whether it pushed everything: it clears the error and advances `lastSyncedAt` on success (including an error a failed pull latched), and claims nothing when it could not push, leaving the entries queued as `pendingCount` | `sync_provider_test.dart`: `"a recovered drain returns the device to idle and backed up"` (latches the error through a failed `syncNow` pull, the only path that reaches it in production), `"a drain that is still failing claims no backup"`, `"a best-effort photo file failure is not a drain failure"` |
...
| 2026-09-11 | The debounced drain now answers from whether it pushed everything, not from the status the previous run latched: a drain that pushed the queue clears the stale "Sync error" and advances "Last synced" (including an error a failed pull latched), and a drain that could not push claims no backup and leaves the work reported as pending. 3 regression cases added |
| 2026-09-14 | Queued edits refresh the backup pending count before the debounce or upload completes, and the Settings tile reports that count whether the last sync idled, failed, or is still waiting on a push the device cannot deliver — a running sync re-reads it once its push half is done, so the pull half no longer reports work already delivered, and a pull that cannot reach the server is captioned in plain words instead of the Firestore SDK's text, which the next edit's count refresh no longer erases. Full and incremental pulls require server reads for all collections, so an unavailable server cannot record a successful sync or advance the pull watermark. 23 regression cases added, 22 of them reproduced at this commit's base |
