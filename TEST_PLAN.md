...
| `test/features/settings/settings_sync_tile_test.dart` | Sync tile against the real notifier, service and queue: an offline launch that cannot pull still reports the work waiting on the device, a later offline edit moves that count before any drain runs, and the failure reason stays alongside it |
...
|---|---|---|
| `fix(sync): keep backup claims honest while offline` | An edit kept the old zero pending count until the drain finished, including while an offline upload waited indefinitely; cache-backed pulls reported success and advanced the per-device watermark past unseen remote edits; and the count the fix keeps current was drawn only in the idle branch of the Settings tile, so the offline error state — now the routine one — showed no sign of the unsent edit | `sync_queue_durability_test.dart`: offline edit before debounce and during upload; `sync_offline_pull_test.dart`: real notifier, service, queue and Drift, with SDK-boundary cache/server behavior for every collection in full and incremental pulls, unchanged watermark/last-sync time on failure, and recovery after reconnect; `settings_sync_tile_test.dart`: the pending count on the tile in the error state. All 19 new cases failed before the fix |
...
| 2026-09-14 | Queued edits refresh the backup pending count before the debounce or upload completes, and the Settings tile reports that count whether the last sync idled or failed. Full and incremental pulls require server reads for all collections, so an unavailable server cannot record a successful sync or advance the pull watermark. 19 regression cases added and reproduced before the fix |
