...
| `test/providers/sync_queue_durability_test.dart` | Real Drift + `PieceWriter` + persisted queue: edits during incremental/full-sync pull or upload retry survive and receive an owed drain; an edit merged into the entry being pushed is not acknowledged by that push; an edit during a drain receives a follow-up; exhausted pushes stay queued without a hot retry loop | 7 |
