...
| 2026-09-14 | Queued edits refresh the backup pending count before the debounce or upload completes, and the Settings tile reports that count whether the last sync idled, failed, or is still waiting on a push the device cannot deliver — a running sync re-reads it once its push half is done, so the pull half no longer reports work already delivered, and a pull that cannot reach the server is captioned in plain words instead of the Firestore SDK's text. Full and incremental pulls require server reads for all collections, so an unavailable server cannot record a successful sync or advance the pull watermark. 22 regression cases added and reproduced before the fix |
