...
| `test/services/sync_service_test.dart` | Push/pull, `pushAllLocal`, `deleteLocalData` (§11 gap: new local stores must be added here) including the key rotation at erase: `PRAGMA rekey` then store, keyed back when the store fails, and every way the device can be left unsecured — an unreadable key store, a rekey the database refuses, a transfer backup that will not delete — reported as such, and only once the photos, watermarks and ownership stamp are gone | 39 |
