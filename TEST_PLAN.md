...
| `test/providers/sync_provider_test.dart` | `SyncState.copyWith` and notifier plumbing, including the debounced drain deciding idle-vs-error from its own outcome rather than the latched status | 45 |
| `test/providers/sync_offline_pull_test.dart` | Real notifier, service, queue and Drift with a cache/server Firestore boundary: unavailable collections or an unconfirmed server boundary prevent success and watermark advancement; slow pulls, clock skew, timestamp precision, client-timed pieces, legacy-marker recovery, failed-migration retries, queued material edits held through the migration pull, a piece deleted mid-pull staying deleted, and an offline empty-queue Sync Now reporting unavailable instead of hanging on the boundary write | 31 |
...
|---|---|---|
| `fix(sync): anchor pull watermarks before server reads` | A pull saved its device-clock completion time and permanently skipped edits arriving after a collection query; a fast device clock skipped additional remote edits. Legacy markers could already lie beyond unseen changes | `sync_offline_pull_test.dart`: full and incremental slow pulls, device clock ahead, legacy marker recovery without stale bulk upload (all four failed before the fix), inclusive timestamp boundary, slow-client piece edits with newer local edits retained, unconfirmed boundaries, failed migration retry, a clay renamed during the migration pull and queued material writes whose push exhausted its retries (both failed before the hold), a piece deleted during an incremental pull staying deleted (failed before the hold), offline Sync Now with nothing queued ending in the unavailable error rather than waiting on the boundary write (failed while the boundary was a plain `set`); `sync_service_test.dart`: a photo's `cloudUrl` uploaded after another device pulled reaches that device (failed before the upload moved `updatedAt`); existing unavailable-collection and pending-edit tests retained. See `docs/pull-watermarks.md` |
