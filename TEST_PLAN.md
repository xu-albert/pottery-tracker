...
|---|---|---|
| `fix(sync): keep backup claims honest while offline` | An edit kept the old zero pending count until the drain finished, including while an offline upload waited indefinitely; cache-backed pulls reported success and advanced the per-device watermark past unseen remote edits | `sync_queue_durability_test.dart`: offline edit before debounce and during upload; `sync_offline_pull_test.dart`: real notifier, service, queue and Drift, with SDK-boundary cache/server behavior for every collection in full and incremental pulls, unchanged watermark/last-sync time on failure, and recovery after reconnect. All 16 new cases failed before the fix |
...
| 2026-09-14 | Queued edits refresh the backup pending count before the debounce or upload completes. Full and incremental pulls require server reads for all collections, so an unavailable server cannot record a successful sync or advance the pull watermark. 16 regression cases added and reproduced before the fix |
